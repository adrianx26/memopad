# AGENTS.md - MemoPad Project Guide

## Project Overview

MemoPad is a local-first knowledge management system built on the Model Context Protocol (MCP). It enables
...
**Doctor Note:** `just doctor` runs with a temporary HOME/config so it won't touch your local MemoPad settings. It leaves temp dirs in `/tmp` (safe to ignore or remove).
...
- Tool access: `memopad tool` (provides CLI access to MCP tools)
    - Continue: `memopad tool continue-conversation --topic="search"`

**Project Management:**
- List projects: `memopad project list`
- Add project: `memopad project add "name" ~/path`
- Project info: `memopad project info`
- One-way sync (local -> cloud): `memopad project sync`
- Bidirectional sync: `memopad project bisync`
- Integrity check: `memopad project check`

**Cloud Commands (requires subscription):**
- Authenticate: `memopad cloud login`
- Logout: `memopad cloud logout`
- Check cloud status: `memopad cloud status`
- Setup cloud sync: `memopad cloud setup`
- Manage snapshots: `memopad cloud snapshot [create|list|delete|show|browse]`
- Restore from snapshot: `memopad cloud restore <path> --snapshot <id>`

### MCP Capabilities

- MemoPad exposes these MCP tools to LLMs:
  ...
  **Project Management:**
    ...
    - `create_memory_project(project_name, project_path, set_default)` - Create new MemoPad projects
...
MemoPad now supports cloud synchronization and storage (requires active subscription):
...
Key behaviors:
- The local MCP server (`memopad mcp`) automatically uses local routing
...
MemoPad emerged from and enables a new kind of development process that combines human and AI capabilities. Instead
of using AI just for code generation, we've developed a true collaborative workflow:

1. AI (LLM) writes initial implementation based on specifications and context
2. Human reviews, runs tests, and commits code with any necessary adjustments
3. Knowledge persists across conversations using MemoPad's knowledge graph
...
MemoPad has taken AI-Human collaboration to the next level by integrating Claude directly into the development workflow through GitHub:
