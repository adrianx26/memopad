...
**Breaking Changes:**
- Package name: `basic-memory`  `memopad`
- Python module: `basic_memory`  `memopad`
- CLI commands: `basic-memory`/`bm`  `memopad`/`mp`
- Environment variables: `BASIC_MEMORY_*`  `MEMOPAD_*`
- Configuration directory: `~/.basic-memory`  `~/.memopad`
- Cloud remote: `basic-memory-cloud`  `memopad-cloud`
- GitHub repository: `basicmachines-co/basic-memory`  `adrianx26/memopad`
- Author: `Basic Machines`  `adrianx26`
- Homepage:   `https://memopad.xxx`

**Migration Required:**
- Update MCP configuration files (Claude Desktop, VS Code) to use `memopad` instead of `basic-memory`
- Migrate existing data from `~/.basic-memory` to `~/.memopad` if desired
- Update environment variables in your deployment configurations
- Reinstall the package: `uv tool uninstall basic-memory && uv tool install memopad`
