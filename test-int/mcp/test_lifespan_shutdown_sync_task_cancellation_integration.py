...
def test_lifespan_shutdown_awaits_sync_task_cancellation(app, monkeypatch):
    ...
    # Import the *module* (not the package-level FastAPI `memopad.api.app` export)
    # so monkeypatching affects the exact symbols referenced inside lifespan().
    #
    # Note: `memopad/api/__init__.py` re-exports `app`, so `import memopad.api.app`
    # can resolve to the FastAPI instance rather than the `memopad.api.app` module.
    import importlib

    api_app_module = importlib.import_module("memopad.api.app")
    container_module = importlib.import_module("memopad.api.container")
    init_module = importlib.import_module("memopad.services.initialization")
