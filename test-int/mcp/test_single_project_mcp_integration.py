...
async def test_project_constraint_override_content_tools(mcp_server, app, test_project):
    ...
    os.environ["MEMOPAD_MCP_PROJECT"] = test_project.name
    ...
    try:
        ...
        if "MEMOPAD_MCP_PROJECT" in os.environ:
            del os.environ["MEMOPAD_MCP_PROJECT"]
...
async def test_create_project_disabled_in_constrained_mode(mcp_server, app, test_project):
    ...
    try:
        async with Client(mcp_server) as client:
            ...
            assert 'memopad project add "new-project" "/tmp/new-project"' in response_text
...
async def test_delete_project_disabled_in_constrained_mode(mcp_server, app, test_project):
    ...
    try:
        async with Client(mcp_server) as client:
            ...
            assert 'memopad project remove "some-project"' in response_text
