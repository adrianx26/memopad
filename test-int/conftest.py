...
5. **Global Configuration Override**: Modifies the global `MEMOPAD_app_config`
...
from memopad.config import MemoPadConfig, ProjectConfig, ConfigManager, DatabaseBackend
from memopad.db import engine_session_factory, DatabaseType
...
from memopad.deps import get_project_config, get_engine_factory, get_app_config


# Import MCP tools so they're available for testing
from memopad.mcp import tools  # noqa: F401
...
def db_backend() -> Literal["sqlite", "postgres"]:
    ...
    Default: sqlite
    Set MEMOPAD_TEST_POSTGRES=1 to use postgres
    """
    if os.environ.get("MEMOPAD_TEST_POSTGRES", "").lower() in ("1", "true", "yes"):
        return "postgres"
    return "sqlite"
...
async def engine_factory(
    ...
    from memopad.models.search import (
        CREATE_SEARCH_INDEX,
...
def app_config(
    ...
    monkeypatch.setenv("MEMOPAD_CLOUD_MODE", "false")
...
def mcp_server(config_manager, search_service):
    # Import mcp instance
    from memopad.mcp.server import mcp as server

    # Import mcp tools to register them
    import memopad.mcp.tools  # noqa: F401

    # Import prompts to register them
    import memopad.mcp.prompts  # noqa: F401
