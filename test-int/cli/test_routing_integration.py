...
from memopad.cli.main import app as cli_app
...
class TestMcpCommandForcesLocal:
    ...
    def test_mcp_sets_force_local_env(self, monkeypatch):
        """MCP command should set MEMOPAD_FORCE_LOCAL before server starts."""
        # Track what environment variable was set
        env_set_value = []

        # Mock the MCP server run to capture env state without actually starting server
        import memopad.cli.commands.mcp as mcp_mod

        def mock_run(*args, **kwargs):
            env_set_value.append(os.environ.get("MEMOPAD_FORCE_LOCAL"))
