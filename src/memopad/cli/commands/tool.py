...
def write_note(
    ...
    content: Annotated[
        Optional[str],
        typer.Option(
            help="The content of the note. If not provided, content will be read from stdin. This allows piping content from other commands, e.g.: cat file.md | memopad tools write-note"
        ),
    ] = None,
    tags: Annotated[
    ...
    memopad tools write-note --title "My Note" --folder "notes" --content "Note content"

    # Using stdin pipe
    echo "# My Note Content" | memopad tools write-note --title "My Note" --folder "notes"

    # Using heredoc
    cat << EOF | memopad tools write-note --title "My Note" --folder "notes"
    # My Document
