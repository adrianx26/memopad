...
- Uses single "memopad-cloud" remote (not tenant-specific)
...
from memopad.cli.commands.cloud.rclone_installer import is_rclone_installed
from memopad.utils import normalize_project_path
...
def get_bmignore_filter_path() -> Path:
    ...
    Uses ~/.memopad/.bmignore converted to rclone format.
    ...
    from memopad.cli.commands.cloud.bisync_commands import (
...
def get_project_bisync_state(project_name: str) -> Path:
    ...
    """
    return Path.home() / ".memopad" / "bisync-state" / project_name
...
def get_project_remote(project: SyncProject, bucket_name: str) -> str:
    ...
    Returns:
        Remote path like "memopad-cloud:bucket-name/memopad-llc"

    Note:
        The API returns paths like "/app/data/memopad-llc" because the S3 bucket
    ...
    return f"memopad-cloud:{bucket_name}/{cloud_path}"
