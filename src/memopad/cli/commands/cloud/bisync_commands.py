...
from memopad.cli.commands.cloud.api_client import make_api_request
from memopad.config import ConfigManager
from memopad.ignore_utils import create_default_bmignore, get_bmignore_path
from memopad.schemas.cloud import MountCredentials, TenantMountInfo
...
def convert_bmignore_to_rclone_filters() -> Path:
    ...
    Reads ~/.memopad/.bmignore (gitignore-style) and converts to
    ~/.memopad/.bmignore.rclone (rclone filter format).
    ...
    # Create rclone filter path: ~/.memopad/.bmignore -> ~/.memopad/.bmignore.rclone
