...
from memopad.config import MemoPadConfig, ConfigManager, DatabaseBackend
...
class DatabaseType(Enum):
    ...
    @classmethod
    def get_db_url(
        cls, db_path: Path, db_type: "DatabaseType", config: Optional[MemoPadConfig] = None
    ) -> str:
...
def _configure_sqlite_connection(dbapi_conn, enable_wal: bool = True) -> None:
    ...
    try:
        ...
        cursor.execute("PRAGMA analysis_limit=1000")  # Analyze up to 1000 rows for statistics
...
def _create_postgres_engine(db_url: str, config: MemoPadConfig) -> AsyncEngine:
    """Create Postgres async engine with appropriate configuration.

    Args:
        db_url: Postgres connection URL (postgresql+asyncpg://...)
        config: MemoPadConfig with pool settings
...
def _create_engine_and_session(
    ...
    config: Optional[MemoPadConfig] = None,
...
async def run_migrations(
    app_config: MemoPadConfig, database_type=DatabaseType.FILESYSTEM
):  # pragma: no cover
