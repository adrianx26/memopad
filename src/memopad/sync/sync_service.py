...
from memopad.ignore_utils import IgnoreMatcher, load_bmignore_patterns
...
class SyncService:
    ...
    def _reload_ignore_patterns(self, directory: Path) -> None:
        """Reload ignore patterns specifically for a directory. Useful for tests."""
        ...
        for line in stdout.decode().splitlines():
            if line:
                try:
                    ...
                    if self._ignore_matcher.match(abs_path.relative_to(directory)):
    ...
    async def scan_directory(self, directory: Path) -> AsyncIterator[Tuple[str, os.stat_result]]:
        ...
        for entry in entries:
            ...
            if self._ignore_matcher.match(entry_path.relative_to(directory)):
