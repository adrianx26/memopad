...
from memopad.ignore_utils import load_gitignore_patterns, IgnoreMatcher
...
class WatchService:
    def __init__(
        ...
        self._ignore_matchers_cache: dict[Path, IgnoreMatcher] = {}
    ...
    def _get_ignore_matcher(self, project_path: Path) -> IgnoreMatcher:
        """Get or create ignore matcher for a project path."""
        if project_path not in self._ignore_matchers_cache:
            ...
            patterns = self._ignore_patterns_cache[project_path]
            self._ignore_matchers_cache[project_path] = IgnoreMatcher(patterns)
        return self._ignore_matchers_cache[project_path]
                ...
                for project in projects:
                    if self.is_project_path(project, path):
                        ...
                        try:
                            ...
                            matcher = self._get_ignore_matcher(project_path)
                            if matcher.match_path_parts(rel_path):
                                logger.trace(
                                    f"Ignoring watched file change: {rel_path}"
                                )
                                continue
                        except ValueError:
                            # Not relative to project path
                            continue
    ...
    async def run(self):  # pragma: no cover
        ...
        try:
            while self.state.running:
                ...
                self._ignore_matchers_cache.clear()
