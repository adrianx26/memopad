"""Configuration management for memopad."""
...
@dataclass
class ProjectConfig:
    """Configuration for a specific memopad project."""
...
class MemoPadConfig(BaseSettings):
    ...
    # overridden by ~/.memopad/config.json
    log_level: str = "INFO"
    ...
    def is_test_env(self) -> bool:
        ...
        - MEMOPAD_ENV environment variable is "test"
...
class ConfigManager:
    ...
    def __init__(self) -> None:
        ...
        if config_dir := os.getenv("MEMOPAD_CONFIG_DIR"):
    ...
    def load_config(self) -> MemoPadConfig:
        ...
        if self.config_file.exists():
            try:
                ...
                # The env_prefix is "MEMOPAD_" so we check those
                for field_name in MemoPadConfig.model_fields.keys():
                    env_var_name = f"MEMOPAD_{field_name.upper()}"
...
def get_project_config(project_name: Optional[str] = None) -> ProjectConfig:
    ...
    os_project_name = os.environ.get("MEMOPAD_PROJECT", None)
    if os_project_name:  # pragma: no cover
        logger.warning(
            f"MEMOPAD_PROJECT is not supported anymore. Set the default project in the config instead. Setting default project to {os_project_name}"
        )
...
def save_MEMOPAD_config(file_path: Path, config: MemoPadConfig) -> None:
