...
from memopad.markdown import EntityMarkdown
...
from memopad.services.provenance_service import validate_provenance as _validate_provenance
from memopad.services.skill_service import (
    SKILL_ENTITY_TYPE,
    validate_skill_payload as _validate_skill_payload,
)
...
class EntityService(BaseService[EntityModel]):
    ...
    async def detect_file_path_conflicts(
        ...
        from memopad.utils import detect_potential_file_conflicts
    ...
    async def resolve_permalink(
        ...
        """Get or generate unique permalink for an entity with caching.
        ...
        # Phase 1 Optimization #4: Smarter cache keys
        ...
        if markdown and markdown.frontmatter.permalink:
            ...
            if not existing_file_path or existing_file_path == file_path_str:
                ...
                # Cache the result
                self._cache_permalink(cache_key, permalink)
                return permalink
        ...
        if existing_permalink:
            # Cache the result
            self._cache_permalink(cache_key, existing_permalink)
    ...
    def _cache_permalink(self, cache_key: str, permalink: str) -> None:
        """Cache a permalink using 2Q cache."""
    ...
    async def warm_permalink_cache(self, top_n: int = 1000) -> int:
        ...
        Phase 1 Optimization #2: Cache warming for faster cold starts.
        
        Call this during application startup or after bulk operations to improve
        initial hit rates.
        
        Args:
            top_n: Number of most recent entities to pre-load
        
        Returns:
            Number of entries warmed
        """
        logger.info(f"Warming permalink cache with up to {top_n} entries...")
        
        try:
            # Get most recently updated files (likely to be accessed soon)
            # Use lightweight query - we only need file_path and permalink
            recent_entities = await self.repository.find_all_lightweight(limit=top_n)
            
            warmed_count = 0
            for entity in recent_entities:
                # Generate normalized cache key
                cache_key = f"path:{entity.file_path.lower()}"
                
                # Pre-populate cache using 2Q put()
                self._permalink_cache.put(cache_key, entity.permalink)
                warmed_count += 1
            
            logger.info(f"Warmed permalink cache with {warmed_count} entries")
            return warmed_count
    ...
    async def fast_write_entity(
        ...
        # Fail-fast provenance invariant (Tb G5): a derived (L1/L2/L3) memory
        # MUST carry a non-empty `source_entities` so the distillation chain stays
        # reversible to L0. No-op unless `levels_enabled` is on and a level is set,
        # so existing note creation (no `level` field) is completely untouched.
        _validate_provenance(
            entity_metadata,
            levels_enabled=bool(self.app_config and self.app_config.levels_enabled),
        )
        # Fail-fast skill payload invariant (Tb G1): a skill's `skill_status`
        # must be a known lifecycle value and `skill_version` a positive int. No-op
        # unless `skills_enabled` is on and the entity is a skill, so ordinary note
        # creation is completely untouched.
        if schema.entity_type == SKILL_ENTITY_TYPE:
            _validate_skill_payload(
                entity_metadata,
                skills_enabled=bool(self.app_config and self.app_config.skills_enabled),
            )
        update_data = {
            "title": schema.title,
    ...
    async def fast_edit_entity(
        ...
        if has_frontmatter(new_content):
            ...
            entity_metadata = {k: v for k, v in metadata.items() if v is not None}
            # Fail-fast provenance invariant (Tb G5) on the fast-edit path too.
            _validate_provenance(
                entity_metadata,
                levels_enabled=bool(self.app_config and self.app_config.levels_enabled),
            )
            # Fail-fast skill payload invariant (Tb G1) on the fast-edit path.
            effective_type = update_data.get("entity_type", entity.entity_type)
            if effective_type == SKILL_ENTITY_TYPE:
                _validate_skill_payload(
                    entity_metadata,
                    skills_enabled=bool(self.app_config and self.app_config.skills_enabled),
                )
