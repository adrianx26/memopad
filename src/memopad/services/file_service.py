...
from memopad import file_utils

if TYPE_CHECKING:  # pragma: no cover
    from memopad.config import MemoPadConfig
from memopad.file_utils import FileError, FileMetadata, ParseError
...
from memopad.services.exceptions import FileOperationError
from memopad.utils import FilePath
...
class FileService:
    ...
    async def get_file_metadata(self, path: FilePath, use_cache: bool = True) -> FileMetadata:
        """Return file metadata for a given path with TTL-based caching.

        This method uses a TTL cache to avoid redundant stat() calls during sync operations.
        Cache entries expire after 60 seconds to balance performance with freshness.
