...
def test_get_project_remote():
    ...
    assert get_project_remote(project, "my-bucket") == "memopad-cloud:my-bucket/research"
...
def test_get_project_bisync_state():
    ...
    expected = Path.home() / ".memopad" / "bisync-state" / "research"
    assert state_path == expected


def test_bisync_initialized_false_when_not_exists(tmp_path, monkeypatch):
    monkeypatch.setattr(
        "memopad.cli.commands.cloud.rclone_commands.get_project_bisync_state",
...
def test_project_sync_success(tmp_path):
    ...
    assert cmd[3] == "memopad-cloud:my-bucket/research"
...
def test_project_ls_with_subpath():
    ...
    cmd, kwargs = runner.calls[0]
    assert cmd[-1] == "memopad-cloud:my-bucket/research/subdir"
