...
async def test_resolve_relations_deletes_duplicate_unresolved_relation(
    ...
    """
    from memopad.models import Relation
...
@pytest.fixture
def test_config_update_permamlinks_on_move(app_config) -> MemoPadConfig:
...
async def test_sync_permalink_updated_on_move(
    test_config_update_permamlinks_on_move: MemoPadConfig,
    project_config: ProjectConfig,
    sync_service: SyncService,
    file_service: FileService,
):
...
async def test_scan_directory_respects_ignore_patterns(
    ...
    sync_service._ignore_matcher = IgnoreMatcher(load_gitignore_patterns(project_dir))
...
async def test_scan_directory_empty_directory(
    ...
    # Directory exists but has no user files (may have .memopad config dir)
