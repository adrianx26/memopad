...
def isolated_home(tmp_path, monkeypatch) -> Path:
    ...
    This prevents tests from reading/writing to ~/.memopad/.bmignore
    ...
    # Set to tmp_path directly (not tmp_path/memopad) so default project
    # home is tmp_path - tests expect to find imported files there
    monkeypatch.setenv("MEMOPAD_HOME", str(tmp_path))
    return tmp_path
