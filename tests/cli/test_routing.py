...
class TestForceRouting:
    ...
    def test_local_sets_env_var(self):
        """Local flag should set MEMOPAD_FORCE_LOCAL."""
        # Ensure env var is not set
        os.environ.pop("MEMOPAD_FORCE_LOCAL", None)

        with force_routing(local=True):
            assert os.environ.get("MEMOPAD_FORCE_LOCAL") == "true"

        # Should be cleaned up after context exits
        assert os.environ.get("MEMOPAD_FORCE_LOCAL") is None

    def test_cloud_clears_env_var(self):
        """Cloud flag should clear MEMOPAD_FORCE_LOCAL if set."""
        # Set env var
    ...
    def test_preserves_original_env_var(self):
        ...
        os.environ["MEMOPAD_FORCE_LOCAL"] = original_value

        with force_routing(local=True):
            assert os.environ.get("MEMOPAD_FORCE_LOCAL") == "true"

        # Should restore original value
        assert os.environ.get("MEMOPAD_FORCE_LOCAL") == original_value
