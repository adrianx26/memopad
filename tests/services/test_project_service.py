...
def test_current_project_property(project_service: ProjectService):
    ...
    original_env = os.environ.get("MEMOPAD_PROJECT")

    try:
        # Test with environment variable not set
        if "MEMOPAD_PROJECT" in os.environ:
            del os.environ["MEMOPAD_PROJECT"]
        ...
        os.environ["MEMOPAD_PROJECT"] = "test-project"
        ...
        if original_env is not None:
            os.environ["MEMOPAD_PROJECT"] = original_env
        elif "MEMOPAD_PROJECT" in os.environ:
            del os.environ["MEMOPAD_PROJECT"]
...
async def test_add_project_with_project_root_sanitizes_paths(
    ...
    """Test that MEMOPAD_PROJECT_ROOT uses sanitized project name, ignoring user path.
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        monkeypatch.setenv("MEMOPAD_PROJECT_ROOT", str(project_root_path))
...
async def test_add_project_with_project_root_rejects_escape_attempts(
    ...
    """Test that MEMOPAD_PROJECT_ROOT rejects paths that try to escape the project root."""
    with tempfile.TemporaryDirectory() as temp_dir:
        # Set up project root environment
        project_root_path = Path(temp_dir) / "app" / "data"
        project_root_path.mkdir(parents=True, exist_ok=True)
...
async def test_add_project_without_project_root_allows_arbitrary_paths(
    ...
    """Test that without MEMOPAD_PROJECT_ROOT set, arbitrary paths are allowed."""
    with tempfile.TemporaryDirectory() as temp_dir:
        # Ensure project_root is not set
        if "MEMOPAD_PROJECT_ROOT" in os.environ:
            monkeypatch.delenv("MEMOPAD_PROJECT_ROOT")
...
async def test_add_project_with_project_root_normalizes_case(
    ...
    """Test that MEMOPAD_PROJECT_ROOT normalizes paths to lowercase.
...
async def test_add_project_with_project_root_detects_case_collisions(
    ...
    """Test that MEMOPAD_PROJECT_ROOT detects case-insensitive path collisions.
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        await project_service.add_project(first_project, "documents/memopad")
        ...
        assert (
            ...
            == (project_root_path / "documents" / "memopad").resolve()
        )
        ...
        try:
            ...
            await project_service.add_project(second_project, "documents/memopad")
...
async def test_add_project_nested_validation_with_project_root(
    ...
    """Test that nested path validation works with MEMOPAD_PROJECT_ROOT set."""
    # Use a completely separate temp directory to avoid fixture conflicts
    with tempfile.TemporaryDirectory() as temp_dir:
        project_root_path = Path(temp_dir) / "app" / "data"
        project_root_path.mkdir(parents=True, exist_ok=True)
