...
from memopad.config import MemoPadConfig, DatabaseBackend
from memopad.repository.project_repository import ProjectRepository
from memopad.services.initialization import (
...
@pytest.mark.asyncio
async def test_initialize_database_creates_engine_and_allows_queries(app_config: MemoPadConfig):
...
async def test_initialize_database_raises_on_invalid_postgres_config(
    app_config: MemoPadConfig, config_manager
):
...
async def test_reconcile_projects_with_config_creates_projects_and_default(
    app_config: MemoPadConfig, config_manager, config_home
):
    await db.shutdown_db()
    try:
...
async def test_reconcile_projects_with_config_swallow_errors(
    monkeypatch, app_config: MemoPadConfig
):
    ...
    try:
        ...
        monkeypatch.setattr(
            "memopad.services.project_service.ProjectService.synchronize_projects",
            boom,
        )
...
def test_ensure_initialization_runs_and_cleans_up(app_config: MemoPadConfig, config_manager):
